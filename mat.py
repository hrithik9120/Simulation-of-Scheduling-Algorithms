# -*- coding: utf-8 -*-
"""
Created on Wed Apr  3 18:59:50 2019

@author: Manoj Gaikwad
"""
import matplotlib
...
#        plt.annotate(str(p[i])+str(su[i])+str(bt[i])+str(wt[i])+str(tat[i]),(x1+3,85-i*6))
        ...
        if(i==n-1):
            ...
            plt.annotate(str(wtavg/n)+"ms",(20,35))
...
#    plt.annotate((name[loc],at[loc],bt[loc],wt[loc],tt[loc],ntt[loc]),(1,19-i*2 ))    
    for i in range(n):
       if flag==0:
           plt.broken_barh([(0,bt[i])],(0,25),edgecolors='black',facecolor='green')
           a=+bt[i] 
           flag+=1
       else:
    #       a+=bt[i]
           plt.broken_barh([(a,bt[i])],(0,25),edgecolors='black',facecolor='red')
           a+=bt[i]
